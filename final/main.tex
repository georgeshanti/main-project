...
The purpose of this project is to build an alternative to conventional input devices. The proposed system will allow users to manipulate their operating system’s Graphical User Interface with their hands to do virtually anything. From moving around elements on the screen such as windows and manipulating them to zooming in on pages, rotating them and using your hands as your own custom cursor. The project centralizes on implementing an application that employs computer vision algorithms and gesture recognition techniques which in turn results in developing a low cost interface device for interacting with objects in the user interface using hand gestures. Existing systems use methods such as machine language algorithms and neural networks such as a CNN to recognize gestures. The proposed system will use a much simpler and less computationally intensive method to do the same. The system can use existing peripherals like the in-built webcam on laptops or any USB webcam connected to the computer.
